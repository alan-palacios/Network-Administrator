...
class router_usuario:
    def nuevo_usuario(ip, usuario, contrasena, nuevo_usuario, nueva_contrasena, os='cisco_ios'):
        try:
            ...
            return True, {"response": "User %s has been inserted"%nuevo_usuario}
    ...
    def modifica_usuario(ip, usuario, contrasena, nueva_contrasena, os='cisco_ios'):
        try:
            ...
            print(output)
            net_connect.disconnect()
            return True, {"response":"User %s has been updated"%usuario}
        except Exception as e:
            return False, {"response":"Error: %s"%e}
    def elimina_usuario(ip, usuario, contrasena, os='cisco_ios'):
        try:
            net_connect = conexion_router.conectar(ip, usuario, contrasena, os)
            ...
            print(output)
            net_connect.disconnect()
            return True, {"response": "User '%s' has been deleted"%usuario}
        except Exception as e:
            return False, {"response": "Error: %s"%e}
